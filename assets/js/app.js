const App = (() => {
    ...
    setHtml(`
      <section class="hero">
        <div class="intro-panel">
          ...
          <p>按下平台頁面的爬取按鈕後，網站會在瀏覽器中嘗試讀取近 7 天公開資料。若平台阻擋 CORS、需要登入或沒有公開 API，頁面會顯示清楚原因。</p>
          <div class="notice">
        ...
        <nav class="home-actions" aria-label="主要功能">
          ...
          <button class="home-button trend" data-link="#trend">
            ...
            <span class="button-copy">
              ...
              <small>整合 PTT、Dcard、Threads 最新爬取結果</small>
            </span>
          </button>
        </nav>
      </section>
  ...
  function renderPlatform(platformKey) {
    ...
    setHtml(`
      <section id="pdfArea" class="tool-panel">
        ...
        <div class="notice">
          自動爬取採公開資料 best-effort 模式，不使用帳號密碼、不接管登入狀態、不繞過平台限制。若平台阻擋，會在下方顯示原因。
        </div>
        <div class="toolbar">
  ...
  async function runCrawl(platformKey) {
    ...
    document.getElementById("statusArea").innerHTML = `<div class="notice">正在嘗試抓取近 7 天公開文章，若平台阻擋會顯示原因。</div>`;
